...
export default function SelectedSociety() {
    const { selectedSociety } = useParams();
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-800 to-black text-white p-4">
            <div className="mt-8 grid gap-x-6 gap-y-8 grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                {hosted.length > 0 && hosted.map(host => (
                    // Check if host.category matches selectedSociety before rendering the event
                    host.category === selectedSociety && (
                        <Link
                            to={'/host/' + host._id}
                            key={host._id}
                        ...
                        >
                            <div className="p-4">
                                <h2 className="font-bold truncate text-xl">{host.startupName}</h2>
                                {host.photos?.[0] && (
                                    <img
